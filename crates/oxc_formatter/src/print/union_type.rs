//! NOTE: Unlike upstream oxc (which stays on Prettier 3.8-style per-member expansion), this
//! printer follows Prettier 3.9's fit-to-width layout for plain unions: the union may move to
//! the next (indented) line and stay flat when it fits (see `types_split_group` below).
//! Parenthesized and complex-tuple unions keep the shared-group per-member expansion.
