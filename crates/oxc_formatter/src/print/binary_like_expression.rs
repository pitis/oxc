        ...
        match self {
            ...
            Self::Right {
                ...
                let operator_and_right_expression = format_with(|f| {
                    // Vue filter sequences force the break before the `|`
                    // regardless of `operatorPosition` (Prettier's
                    // `lineBeforeOperator`); the own-line-comment case is
                    // excluded by the guard above and takes the normal path.
