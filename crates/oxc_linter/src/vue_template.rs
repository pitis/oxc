...
fn as_vue_template_rule(rule: &RuleEnum) -> Option<&dyn VueTemplateRule> {
    match rule {
        ...
        RuleEnum::VueAttributesOrder(rule) => Some(rule),
        RuleEnum::VueFirstAttributeLinebreak(rule) => Some(rule),
...
pub struct TemplateCommentDirectives {
...
pub fn line_start_offsets(text: &str) -> Vec<u32> {
